...
if run_args.dataset == 'mnist_13rot':
    print("Initialize MNIST_13Rot data and data loaders...")

    from datasets import MNIST_13Rot

    train_data = MNIST_13Rot(train=True, device=run_args.device)
    test_data  = MNIST_13Rot(train=False, device=run_args.device)

    # move data and labels back to cpu for testing or low-VRAM setups
...
#     for swapped_cluster_1 in range(swapped_cluster_0, args.n_clusters_j_list[swapped_facet]):
        ...
        def plot_torch_grid(img_list, nrow, title=None):
            ...
            elif run_args.dataset in ['fast_svhn', 'fast_3dshapes', "simple_2d_gmm"]:
                ax.imshow(grid_img)
            else:
                exit("dataset not known.")
