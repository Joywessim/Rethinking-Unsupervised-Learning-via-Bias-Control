...
from utils import (visualize_latent_space_with_clusters, visualize_latent_space, 
                   cluster_acc_and_conf_mat, cluster_acc_weighted, str2bool, load_args_from_yaml, 
                   align_clusters, align_latent_space_full,plot_latent_spaces)
...
def parse_list(arg):
    """ Helper function to parse comma-separated values into a list of integers """
    return list(map(int, arg.strip("[]").split(',')))

# def visualize_latent_space(mfcvae, args, epoch, beta_z, beta_c):
#     """Run latent space visualization every 25 epochs."""
   

#     print(f"Visualizing latent space at epoch {epoch}...")

#     # Load dataset with true latent factors
#     test_data = Simple2DGMM(num_samples=1000, return_z_true=True)
#     test_loader = torch.utils.data.DataLoader(test_data, batch_size=1000, shuffle=False)

#     # Extract latents
#     z_true_list, z_hat_list, c_true_list, c_hat_list = [], [], [], []

#     for x, z_true, c_true in test_loader:
#         x = x.to("cpu")

#         with torch.no_grad():
#             _, _, z_hat = mfcvae.forward(x, args.n_epochs - 1, 0)
#             c_hat = torch.argmax(mfcvae.compute_q_c_j_x(z_hat)[0], dim=1)

#         z_true_list.append(z_true.numpy())
#         z_hat_list.append(z_hat[0].cpu().numpy())  
#         c_true_list.append(c_true.numpy().flatten())
#         c_hat_list.append(c_hat.cpu().numpy())

#     # Convert to NumPy arrays
#     Z_true = np.vstack(z_true_list)
#     Z_hat = np.vstack(z_hat_list)
#     C_true = np.concatenate(c_true_list)
#     C_hat = np.concatenate(c_hat_list)

#     # C_hat_mapped, mapping = align_clusters(Z_true, C_true, Z_hat, C_hat)
#     Z_hat_aligned, R, S = align_latent_space_full(Z_true, Z_hat)
#     plot_latent_spaces(Z_true, C_true, Z_hat, C_hat, Z_hat_aligned)
...
def train(beta_z, beta_c):
    ...
    parser.add_argument('--dataset', type=str, default='shapes_colors', metavar='N',
    ...
    parser.add_argument('--gamma_kl_z_pretrain', type=float, default=2.0, metavar='N', help='gamma factor used on z KL-terms during the pretraining phase')
    parser.add_argument('--gamma_kl_c_pretrain', type=float, default=1.0, metavar='N', help='gamma factor used on c KL-terms during the pretraining phase')
    ...
    parser.add_argument('--sigma_multiplier_p_x_z', type=float, default=0.3, metavar='N',
    ...
    parser.add_argument('--n_recons_per_cluster', type=int, default=30,
                        metavar='N', help='how many input-recon-pairs per cluster to log')
    parser.add_argument('--n_examples_per_cluster', type=int, default=30,
                        metavar='N', help='how many examples per cluster to log')
    parser.add_argument('--n_sample_generations_per_cluster', type=int, default=30,
    ...
    parser.add_argument('--z_j_dim_list', type=parse_list, default=[5, 5], metavar='N',
                    help='dimension of each z_j variable')
    parser.add_argument('--n_clusters_j_list', type=parse_list, default=[25, 25], metavar='N',
                        help='number of possible values of each c_j')
    parser.add_argument('--n_epochs_per_progressive_step', type=parse_list, default=[201, 201], metavar='N', 
                        help="number of epochs during each progressive training step")

        
    args, unknown = parser.parse_known_args()
    print("args not parsed in train: ", unknown)

    print("z_j_dim_list:", args.z_j_dim_list)
    print("n_clusters_j_list:", args.n_clusters_j_list)
    print("n_epochs_per_progressive_step:", args.n_epochs_per_progressive_step)
    print("J_n_mixtures:", args.J_n_mixtures)
   ...
   # load config dictionary instead
    ...
    print("Initialize Shapes dataset and data loaders...")


    # train_data = RedShapesDataset(num_samples=1000)
    # test_data = RedShapesDataset(num_samples=200)
    ...
    test_loader  = torch.utils.data.DataLoader(test_data, batch_size=args.eval_batch_size, shuffle=True, num_workers=0)

    height, width = 28, 28
    n_true_classes = 2
    in_channels = 1
    
    in_dim = height * width * in_channels
    likelihood_model = 'Bernoulli'
    activation_x_hat_z = "sigmoid"
    n_labels = 1 #our dataset returns one label per image
    ...
    if args.model_type in ['fc_shared', 'fc_per_facet_enc_shared_dec', 'fc_vlae', 'vlae_orig', 'conv_vlae']:
        ...
        # model_path = f"Results/beta_z_{beta_z}_beta_c_{beta_c}/save_dict_model.pt"
        # mfcvae, args = load_model_from_save_dict(model_path, map_location="cpu")
        # mfcvae.eval()
    ...
    print("args.n_epoch= ", args.n_epochs)
    for epoch in range(args.n_epochs):
        ...
        if epoch == 0:
            ...
            for batch_idx, (x, y_true) in enumerate(train_loader):
                ...
                # print(f"After reshaping - x.shape: {x.shape}")  # Debugging print
                ...
                loss, mean_log_prob_p_x_z, mean_log_prob_E_p_z_c, mean_log_prob_E_p_c, mean_log_prob_E_q_z_x, mean_log_prob_E_q_c_x, kl_z, kl_c = mfcvae.compute_loss_5terms(x, x_hat, q_z_j_x_list, z_sample_q_z_j_x_list, epoch,beta_z, beta_c)
                ...
                if args.do_vis_train and epoch % args.vis_every_n_epochs == 0:
                    visualize_latent_space_with_clusters(mfcvae, train_loader, epoch, device)
                    ...
                    for i in range(mfcvae.J_n_mixtures):
                        ...
                        for j in range(mfcvae.n_clusters_j_list[i]):
                            ...
                            if not vis_recon_logging_list_done[i]:
                                ...
                                n_taken_recon_and_input = min(count_indices, args.n_recons_per_cluster - vis_count_recon_per_cluster[i][j])
                                ...
                                if n_taken_recon_and_input > 0:
                                    ...
                                    for k in indices:
                                        shape = x[k].shape
                                        # print(f"x[{k}] shape: {shape}")  # Debugging print
                                        
                                        if len(shape) == 1 and shape[0] == 28 * 28 * 1:  # Flattened tensor
                                            x_processed = x[k].reshape(1, 28, 28)  # Reshape correctly
                                        elif len(shape) == 3 and shape[0] == 3:  # Already in (C, H, W) format
                                            x_processed = x[k]
                                        else:
                                            raise ValueError(f"Unexpected shape {shape} for x[{k}]")  # Catch errors

                                        vis_x_recon_logged[i][j].append((x_processed.cpu().detach(), f'input_facet_{i}_pred_{j}'))
                                        vis_x_hat_recon_logged[i][j].append((x_processed.cpu().detach(), f'recon_facet_{i}_pred_{j}'))
        ...
        if args.do_test_during_training or (not args.do_test_during_training and epoch == args.n_epochs - 1):
            with torch.no_grad():
                ...
                for batch_idx, (x, y_true) in enumerate(test_loader):
                    ...
                    # Ensure x is on the same device as the mode
                    x = x.to(device)
                    # 🛠 Fix input shape
                    x = x.view(x.size(0), -1).float()
                        ...
                        for i in range(mfcvae.J_n_mixtures):
                            ...
                            for j in range(mfcvae.n_clusters_j_list[i]):
                                ...
                                if not vis_recon_logging_list_done[i]:
                                    ...
                                    if n_taken_recon_and_input > 0:
                                        # Ensure indices are within range of available data
                                        indices = [k for k in indices[:n_taken_recon_and_input] if k < len(x)]

                                        if len(indices) > 0:  # Only proceed if valid indices exist
                                            vis_x_recon_logged[i][j] = vis_x_recon_logged[i][j] + [
                                                (x[k].cpu().detach().permute(2, 0, 1), 'input_facet_' + str(i) + '_pred_' + str(j)) 
                                                for k in indices
                                            ]
                                            vis_x_hat_recon_logged[i][j] = vis_x_hat_recon_logged[i][j] + [
                                                (x_hat[k].cpu().detach().permute(2, 0, 1), 'recon_facet_' + str(i) + '_pred_' + str(j)) 
                                                for k in indices
                                            ]
                                            vis_count_recon_per_cluster[i][j] += len(indices)
                ...
                if args.do_vis_test and epoch % args.vis_every_n_epochs == 0 and (args.do_vis_examples_per_cluster or args.do_vis_sample_generations_per_cluster):
                    ...
                    fig_list, row_indices, column_indices, num_nonempty_clusters = plot_top10_cluster_examples(test_data, mfcvae, args, epoch)
                ...
                grad_norm = torch.nn.utils.clip_grad_norm_(mfcvae.parameters(), max_norm=float('inf'))



                print_string = "Eval Loss: {:.4f}, Gradient L2 Norm: {:.4f}, ".format(eval_loss, grad_norm.item())
    ...
    # save model
    if args.save_model:
        os.makedirs(f"RGB_Results/beta_z_{beta_z}_beta_c_{beta_c}",exist_ok=True)

        save_dict_path = os.path.join(f"Results/beta_z_{beta_z}_beta_c_{beta_c}", f"save_dict_model.pt")
