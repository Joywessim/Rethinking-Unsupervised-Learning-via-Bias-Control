...
from utils import cluster_acc_and_conf_mat, cluster_acc_weighted, str2bool, load_args_from_yaml, align_clusters, align_latent_space_full,plot_latent_spaces
from datasets import Fast_Circles
...
def train(beta_z, beta_c):
    ...
    parser.add_argument('--dataset', type=str, default='circles', metavar='N',
    ...
    parser.add_argument('--cov_type_p_z_c', type=str, default="full", metavar='N',
    ...
    parser.add_argument('--n_sample_generations_per_cluster', type=int, default=20,
   ...
   # load config dictionary instead
    ...
    if args.dataset == 'circles':
        print("Initialize circles dataset and data loaders...")
        # initialize dataset
        ...
        train_data  = Fast_Circles(n_samples=2000, noise=0.1, device=args.device,n_circles=1)
        test_data  = Fast_Circles(n_samples=500, noise=0.1, device=args.device, n_circles=1)
    ...
    if args.dataset == 'fast_mnist':
        ...
        # Two circles: each sample is a 2D coordinate, and there are 2 classes.
        n_true_classes = 2
        cmap = 'viridis'
        in_channels = 1  # Not really used here, but kept for API consistency.
        in_dim = 2       # Each sample has two features (x and y coordinates).
        likelihood_model = 'Gaussian'  # Coordinates are continuous.
        activation_x_hat_z = None      # No activation is applied by default.
        n_labels = 1
        height, width = 1,2
    ...
    print("activation_x_hat_z=", activation_x_hat_z)
    ...
    # Use StepLR: Reduce LR every 50 epochs by a factor of 0.5
    ...
    for epoch in range(args.n_epochs):
        ...
        # # if epoch %5==0:
        # mfcvae.eval()
        # visualize_latent_space(mfcvae, args, epoch, beta_z, beta_c)
        ...
        if epoch == 0:
            ...
            for batch_idx, (x, y_true) in enumerate(train_loader):
                ...
                if args.dataset == "simple_2d_gmm":
                    x = x.view(-1, 2)  # Since each sample has only 2 values
                else:
                    x = x.view(-1, in_channels, height, width).permute(0, 2, 3, 1)

                if args.dataset == "simple_2d_gmm":
                    x_hat = x_hat.view(-1, 2)  # Match the shape of original input x
                else:
                                ...
                                if n_taken_recon_and_input > 0:
                                    ...
                                    if args.dataset == "simple_2d_gmm":
                                        vis_x_recon_logged[i][j] = vis_x_recon_logged[i][j] + [
                                            (x[k].cpu().detach(),  # ✅ No permute needed for 2D
                                            'input_facet_' + str(i) + '_pred_' + str(j)) for k in indices]
                                        vis_x_hat_recon_logged[i][j] = vis_x_hat_recon_logged[i][j] + [
                                            (x_hat[k].cpu().detach(),  # ✅ No permute needed for 2D
                                            'recon_facet_' + str(i) + '_pred_' + str(j)) for k in indices]
                                    else:
                                        vis_x_recon_logged[i][j] = vis_x_recon_logged[i][j] + [
                                            (x[k].cpu().detach().permute(2, 0, 1),  # ✅ Keep permute for images
                                            'input_facet_' + str(i) + '_pred_' + str(j)) for k in indices]
                                        vis_x_hat_recon_logged[i][j] = vis_x_hat_recon_logged[i][j] + [
                                            (x_hat[k].cpu().detach().permute(2, 0, 1),  # ✅ Keep permute for images
                                            'recon_facet_' + str(i) + '_pred_' + str(j)) for k in indices]
    ...
    # save model
    if args.save_model:
        path = f"Results_one_circle/beta_z_{beta_z}_beta_c_{beta_c}_z_dim_{args.z_j_dim_list}_n_clusters_{args.n_clusters_j_list}_J_{mfcvae.J_n_mixtures}_{likelihood_model}"
        os.makedirs(path,exist_ok=True)

        save_dict_path = os.path.join(path, f"save_dict_model.pt")
