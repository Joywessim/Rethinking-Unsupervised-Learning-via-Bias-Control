...
class MFCVAE(nn.Module):
    ...
    def forward(self, x: torch.tensor, epoch: int, batch_idx: int):
        ...
        q_z_j_x_list = [
            D.Independent(
                D.Normal(
                    loc=mu_q_z_j_x_list[j],
                    scale=torch.sqrt(torch.exp(log_sigma_square_q_z_j_x_list[j]) + 1e-8)
                ),
                1
            )
            for j in range(self.J_n_mixtures)
    ...
    def compute_loss_5terms(self, x: torch.tensor, x_hat: torch.tensor,
                            ...
                            z_sample_q_z_j_x_list: List[torch.tensor], epoch: int, beta_z=1.0, beta_c =1.0 ):
        """
        ...
        # else:
        #     beta_z = 4.0  #  if as in ProVLAE implementation: self.beta_z
